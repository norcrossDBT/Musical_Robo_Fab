...
read_data_Path = data_Path  + "\data\JSON_files"
filename = os.path.join(read_data_Path, JSON_Name)
...
note_time = JSON_dict['fab_time'] / len(JSON_dict['fab_dists'])

# playback tones heard during fabrication
for freq in JSON_dict['fab_tones']:
    if freq == 700 or 100:
        player.play_wave(synthesizer2.generate_constant_wave(freq, note_time))
        wave.append(synthesizer2.generate_constant_wave(freq, note_time))
    else:
        player.play_wave(synthesizer.generate_constant_wave(freq, note_time * 2))
        wave.append(synthesizer.generate_constant_wave(freq, note_time * 2))
